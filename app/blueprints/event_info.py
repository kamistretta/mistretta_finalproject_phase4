...
def update_event(event_id):
    db = get_db()
    event = db.execute('SELECT * FROM event_info WHERE event_id = %s', (event_id,)).fetchone()
    ...
    if request.method == 'POST':
        ...
        db.execute('UPDATE event_info SET event_name = %s, category = %s, description = %s, date = %s, time = %s, location = %s, max_attendees = %s WHERE event_id = %s',
                   (event_name, category, description, event_date, event_time, location, max_attendees, event_id))
        db.commit()
        return redirect(url_for('event_info.events'))  # Redirect back to events page

    return render_template('events_update.html', current_event_info=event)
